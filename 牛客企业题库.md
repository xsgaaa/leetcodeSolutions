	...
	while (getline(cin,strN))
	{
		...
		for (int i = 0; i < N; i++)
		{
			...
			while (st < s.size())
			{
				...
				if (pos == string::npos)
				{
					V[i].push_back(stoi(s.substr(st)));
                    break;
				}
				else
				{
					V[i].push_back(stoi(s.substr(st,pos-st)));
					st = pos + 1;
				}
			}
            for(auto &v:V[i])
            {
                if(mp.count(v)==0)
                    mp[v]=++idx;//标记每个数第一次出现的位置
                v=mp[v];		//并用这个位置来代替原来的值
            }					//相当于离散化数据
		}
        int fa[idx+2];
        ...
        for(int i=1;i<=idx;i++)
        {
            fa[i]=i;		//代表每个数属于哪个集合，进行初始化
            cnt[i]=1;		//每个集合的元素应该不重复
        }
        for(int i=0;i<N;i++)
        {
            int m=V[i].size();
            int u=find(fa,V[i][0]);		//查询第一个元素所在的集合
            for(int j=1;j<m;j++)
            {
                int v=find(fa,V[i][j]);	//查询当前集合内其他元素所在的集合，如果不在一个集合，就合并到第一个集合中
                if(v!=u)
                {
                    fa[v]=u;
                    cnt[u]+=cnt[v];		//集合合并的时候，要加上原有集合的数量
                }
            }
        }
        int res=0;
        ...
        for(int i=1;i<=idx;i++)
        {
            fa[i]=find(fa,i);			//查找当前元素所属集合
            maxcnt=max(maxcnt,cnt[i]);	//计算集合中所包含的最多的元素
            if(fa[i]==i)    res++;		//如果所属集合于当前索引重合，则为一个新集合
        }
        cout<<res<<endl<<maxcnt<<endl;
	}
	return 0;
}
```
...
void  dfs(int st,int cur,int & res)
{
    ...
    int num=0;		//num表示连续几位的数字
    for(int i=st;i<=N;i++)
    {
        num+=i;
        dfs(i+1,cur+num,res);  		//加上
        if(st>1)//不存在一个数，能减去第一个数，所以要跳过
            dfs(i+1,cur-num,res);	//减去
        num*=10;
    }
}
int main()
